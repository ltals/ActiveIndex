...
signalBasic <- function(longdf){
  longdf <- longdf %>%
  mutate(monthRel = format(dateRel, "%m-%Y")) %>%
  mutate(quarterRel = paste0(year(dateRel), "-", quarter(dateRel))) %>% 
  group_by(series_id) %>% 
  mutate(logDelta = log(value/lag(value)),
         threeMA = rollapply(value, 3, mean, fill = NA, align = "right")) %>%
  group_by(quarterRel, series_id) %>% 
  summarise(logSum = sum(logDelta),
            value = last(value),
            threeMA = last(threeMA)) %>% 
  mutate(xOver = case_when(value >= threeMA ~ 1,
                           value < threeMA ~ -1))
...
energy <- tidyquant::tq_get(energy_series,
                            ...
                            from = "2000-01-01", 
                            to = Sys.Date()) %>% 
  dplyr::select(date, series_id = symbol, value = adjusted) %>% 
...
energy <- rbind(energy, gpr) %>%
  mutate(monthRel = format(date, "%m-%Y")) %>% 
  group_by(series_id) %>% 
  mutate(logDelta = log(value/lag(value)))
...
conDiscSeries <- c("UMCSENT", "UNRATE", "PCE")#, "DGORDER")
...
finSeries <- c("HOUST", "DGS3MO") #TOTALSL
