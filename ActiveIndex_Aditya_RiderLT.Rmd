...
COMPUSTAT_Updated_w <- full_join(COMPUSTAT_Updated_w, ratioAvg, by = c("Industry", "date")) %>% 
  group_by(symbol) %>%
  fill(colnames(Financials), .direction = "down") %>%
  ungroup() 
...
topThirdComp %>% 
  ...
  mutate(logRet = log(Close/qOpen)) %>% 
  ungroup() %>% 
  summarise(mean(logRet))

test2 <- COMPUSTAT_Updated_w %>%
  group_by(Industry, date) %>%  
  mutate(ntileSum = ntile(sigSum, 3)) %>%
  ungroup() %>% 
  filter(date > '2000-12-01')  %>% 
...
COMPUSTAT_Updated_w %>%
  arrange(date) %>% 
  group_by(Industry, date)  %>%  
  filter(Industry == "Materials") %>% 
  mutate(ntileSum = ntile(sigSum, 3)) %>%
  ungroup() %>% 
  filter(date > '2000-12-01') %>% 
  mutate(QBeginMrktCap = qOpen*qBegin_cshoc) %>% 
  ungroup() %>% 
  group_by(date) %>% 
  mutate(qBeginSumMrktCap = sum(QBeginMrktCap),
         weight = QBeginMrktCap/qBeginSumMrktCap) %>% 
  group_by(symbol) %>% 
  mutate(logRet = log(Close/qOpen)) %>% 
  mutate(weightedLogRet = logRet*weight) %>% 
  group_by(date) %>% 
  filter(symbol == "AGI")

sectorList <- unique(COMPUSTAT_Updated_w$Industry)


qRetSectorFiltered <- function(sector){
  COMPUSTAT_Updated_w %>%
  arrange(date) %>%
  filter(symbol != "AGI") %>% 
  group_by(Industry, date)  %>%  
  ...
  mutate(weightedLogRet = logRet*weight) %>% 
  group_by(date) %>% 
  summarise(qLogRet = sum(weightedLogRet)) %>% 
  mutate(sector = sector)
}

retBySectorFiltered <- data.frame()

for (sector in sectorList) {
  retBySectorFiltered <- rbind(retBySectorFiltered, qRetSectorFiltered(sector))
}
  

qRetSector <- function(sector){
  COMPUSTAT_Updated_w %>%
  arrange(date) %>%
  filter(symbol != "AGI") %>% 
  group_by(Industry, date)  %>%  
...
retBySector <- data.frame()

for (sector in sectorList) {
  retBySector <- rbind(retBySector, qRetSector(sector))
}
